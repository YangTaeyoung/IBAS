...
def index(request):
    # 임시 로그인
    session.save_session(request, User.objects.get(pk=12192199))
    if is_chief_exist():
...
def activity(request):
    ...
    board_list = Board.objects.filter(board_type_no__board_type_no=4).order_by('-board_created').prefetch_related("boardfile_set")
