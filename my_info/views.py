...
def my_info(request):
    if request.session.get("user_stu") is not None:
        ...
        my_comment_list = Comment.objects.filter(comment_writer=my_info).select_related("board").select_related(
            "board_type").order_by("comment_board_no__board_type_no").order_by("-comment_created")
